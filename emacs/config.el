...
;; Настройка темы tango
(load-theme 'tango t)

;; Настройка цвета фрейма (необязательно, tango уже задаёт цвета)
(setq default-frame-alist 
      (append default-frame-alist
              '((background-color . "#fdf6e3")  ; Можешь поменять под нужное
                (foreground-color . "#333333"))))

;; Установка фоновых цветов вручную (опционально)
...
(when (system-is-mac)
  (with-eval-after-load "ispell"
    ...
    (setq ispell-local-dictionary-alist
      '(
        ("en-us" "[[:alpha:]]" "[^[:alpha:]]" "[']" nil ("-d" "en-us") nil utf-8)
        ("ru-ru" "[[:alpha:]]" "[^[:alpha:]]" "[']" nil ("-d" "ru-ru") nil utf-8)
        ("el-gr" "[[:alpha:]]" "[^[:alpha:]]" "[']" nil ("-d" "el-gr") nil utf-8)
      ))
